...
{
    statbuf->dev = inode->dev;             // 文件所在设备号
    statbuf->nr = inode->nr;               // 文件i节点号
    statbuf->mode = inode->desc->mode;     // 文件属性
    statbuf->nlinks = inode->desc->nlinks; // 文件的链接数
    statbuf->uid = inode->desc->uid;       // 文件的用户id
    statbuf->gid = inode->desc->gid;       // 文件的组id
