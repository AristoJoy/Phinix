...
typedef struct pci_command_t
{
    ...
    u8 bus_master : 1;
    u8 special_cycles : 1;
    u8 memory_write_invalidate_enbale : 1;
...
} _packed pci_statue_t;

u32 pci_in_dword(u8 bus, u8 dev, u8 func, u8 addr)
{
    out_dword(PCI_CONF_ADDR, PCI_ADDR(bus, dev, func, addr));
    return in_dword(PCI_CONF_DATA);
}
...
void pci_out_dword(u8 bus, u8 dev, u8 func, u8 addr, u32 value)
{
    out_dword(PCI_CONF_ADDR, PCI_ADDR(bus, dev, func, addr));
    out_dword(PCI_CONF_DATA, value);
}
...
int pci_find_bar(pci_device_t *device, pci_bar_t *bar, int type)
{
    for (size_t idx = 0; idx < PCI_BAR_NR; idx++)
    {
        ...
        u32 value = pci_in_dword(device->bus, device->dev, device->func, addr);
        pci_out_dword(device->bus, device->dev, device->func, addr, -1);
        u32 len = pci_in_dword(device->bus, device->dev, device->func, addr);
        pci_out_dword(device->bus, device->dev, device->func, addr, value);
...
static void pci_check_device(u8 bus, u8 dev)
{
    ...
    {
        value = pci_in_dword(bus, dev, func, PCI_CONF_VENDOR);
        u32 vendorid = value & 0xffff;
        ...
        value = pci_in_dword(bus, dev, func, PCI_CONF_COMMAND);
        pci_command_t *cmd = (pci_command_t *)&value;
        pci_statue_t *status = (pci_statue_t *)(&value + 2);

        value = pci_in_dword(bus, dev, func, PCI_CONF_REVISION);
...
{
    u32 data = pci_in_dword(device->bus, device->dev, device->func, PCI_CONF_INTERRUPT);
    return data & 0xff;
}

void pci_enable_busmastering(pci_device_t *device)
{
    u32 data = pci_in_dword(device->bus, device->dev, device->func, PCI_CONF_COMMAND);
    data |= PCI_COMMAND_MASTER;
    pci_out_dword(device->bus, device->dev, device->func, PCI_CONF_COMMAND, data);
}
