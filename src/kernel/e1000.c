...
static void e1000_handler(int vector)
{
    ...
    u32 status = mem_in_dword(e1000->membase + E1000_ICR);
    ...
    if ((status & IM_TXDW))
    {
        ...
        u32 cur = (mem_in_dword(e1000->membase + E1000_TDH) - 1) % TX_DESC_NR;
    ...
    if (status & IM_LSC)
    {
        ...
        // u32 e1000_status = mem_in_dword(e1000->membase + E1000_STATUS);
...
{
    mem_out_dword(e1000->membase + E1000_EERD, 0x1);
    for (size_t i = 0; i < 1000; i++)
    {
        u32 value = mem_in_dword(e1000->membase + E1000_EERD);
        if (value & 0x10)
        {
            e1000->eeprom = true;
        }
...
static u16 e1000_eeprom_read(e1000_t *e1000, u8 addr)
{
    ...
    {
        mem_out_dword(e1000->membase + E1000_EERD, 1 | (u32)addr << 2);
        while (!((tmp = mem_in_dword(e1000->membase + E1000_EERD)) & (1 << 1)))
            ;
    }
...
static void e1000_read_mac(e1000_t *e1000)
{
    ...
    if (e1000->eeprom)
    {
        ...
        value = e1000_eeprom_read(e1000, 1);
        e1000->mac[2] = value & 0xFF;
        e1000->mac[3] = value >> 8;

        value = e1000_eeprom_read(e1000, 2);
        e1000->mac[4] = value & 0xFF;
        e1000->mac[5] = value >> 8;
    }
    else
    {
...
static void e1000_reset(e1000_t *e1000)
{
    ...
    // 接收描述符头尾指针
    mem_out_dword(e1000->membase + E1000_RDH, 0);
    mem_out_dword(e1000->membase + E1000_RDT, RX_DESC_NR - 1);
    ...
    mem_out_dword(e1000->membase + E1000_RCTL, value);
    ...
    // 传输描述符头尾指针
    mem_out_dword(e1000->membase + E1000_TDH, 0);
    mem_out_dword(e1000->membase + E1000_TDT, 0);
    ...
    mem_out_dword(e1000->membase + E1000_TCTL, value);

    // 初始化中断
    value = 0;
    value = IM_RXT0 | IM_RXO | IM_RXDMT0 | IM_RXSEQ | IM_LSC;
    value |= IM_TXQE | IM_TXDW | IM_TXDLOW;
    mem_out_dword(e1000->membase + E1000_IMS, value);
}
...
void e1000_init()
{
    ...
    // 映射内存应该在告地状，避免与用户程序地址和操作系统冲突
    // 但不在最后4M内，最后4M用于页表
    // 不然内存映射机制有问题，需要跳转，不鞥直接使用map_area
