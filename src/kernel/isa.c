...
#define ISA_DMA_CHAN2_READ 0x46 // 通道2 单次dma 读
#define ISA_DMA_CHAN2_WRITE 0x4A // 通道2 单次dma 写

#define ISA_DMA0_CHAN2_PAGE 0x81 // 通道2
...
void isa_dma_addr(u8 channel, void *addr)
{
    ...
    if (channel >= 5)
    {
        offset >>= 1; // 16位通道，地址需要往右移一位
    }
    // 触发器获取端口数据
    out_byte(port, offset & 0xFF);
    out_byte(port, (offset >> 8) & 0xFF);

    // 设置页寄存器
    port = DMA_PAGE[channel];
    out_byte(port, (u32)addr >> 16);
}
...
void isa_dma_size(u8 channel, u32 size)
{
    ...
    if (channel > 5)
    {
        size >= 1; // 16位通道，大小往右移一位
    }
    out_byte(port, (size - 1) & 0xFF);
    out_byte(port, ((size - 1) >> 8) & 0xFF);
}
