...
static err_t fd_outb(u8 byte)
{
    ...
    while (true)
    {
        if (timer_is_expires(expire))
            return -ETIME;
        u8 msr = in_byte(FDC_MSR) & (MSR_READY | MSR_DIO);
        LOGK("out state 0x%X %d %d...\n", msr, expire, jiffies);
        if (msr == MSR_READY)
        {
            out_byte(FDC_DATA, byte);
            return EOK;
        }
        task_yield();
    }
...
static err_t fd_inb()
{
    ...
    while (true)
    {
        if (timer_is_expires(expire))
            return -ETIME;
        u8 msr = in_byte(FDC_MSR) & (MSR_READY | MSR_DIO | MSR_BUSY);
        if (msr == (MSR_READY | MSR_DIO | MSR_BUSY))
            return in_byte(FDC_DATA) & 0xFF;
        task_yield();
    }
}
...
static void fd_result(floppy_t *fd, bool sensei)
{
    ...
    if (in_byte(FDC_DIR) & DIR_CHANGED)
        fd->changed = true;
}
...
static err_t fd_read(floppy_t *fd, void *buf, u8 count, idx_t lba)
{
    ...
    while (left)
    {
        if ((ret = fd_transfer(fd, FD_READ, buf, count, lba)) < EOK)
            break;
        left -= ret;
        lba += ret;
    }
...
static err_t fd_write(floppy_t *fd, void *buf, u8 count, idx_t lba)
{
    ...
    while (left)
    {
        if ((ret = fd_transfer(fd, FD_WRITE, buf, count, lba)) < EOK)
            break;
        left -= ret;
        lba += ret;
    }
...
{
    LOGK("floppy setup...\n");
    out_byte(FDC_DOR, DOR_IRQ | (fd->drive) | (0x10 << fd->drive));
    // assert(fd_wait(fd) == EOK);
    return EOK;
}
