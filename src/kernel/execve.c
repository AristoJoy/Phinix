...
{
    u8 ei_magic[4];    // 内容为0x7F, ELF
    u8 ei_class;       // 文件种类,1-32为，2-64位
    u8 ei_data;        // 标记大小端 1-小端 2-大端
    u8 ei_version;     // 与e_version一样，必须为1
    u8 ei_pad[16 - 7]; // 占满16个字节
} _packed ELFIdent;

// ELF文件头
typedef struct Elf32_Ehdr
{
    ELFIdent e_ident;       // ELF文件标记，文件最开始的16个字节
    Elf32_Half e_type;      // 文件类型，见Etype
    Elf32_Half e_machine;   // 处理机架构类型，标记运行的CPU，见EMachine
    Elf32_Word e_version;   // 文件版本，见EVersion
    Elf32_Addr e_entry;     // 程序入口地址
    Elf32_Off e_phoff;      // program header offset 程序头表在文件中的偏移量
    Elf32_Off e_shoff;      // section header offset 节头标在文件中的偏移量
    Elf32_Word e_flags;     // 处理器特殊标记
    Elf32_Half e_ehsize;    // ELF header size ELF文件头大小
    Elf32_Half e_phentsize; // program header entry size 程序头表项大小
    Elf32_Half e_phnum;     // program header number 程序头表项数量
    Elf32_Half e_shentsize; // section header entry size 节头标项大小
    Elf32_Half e_shnum;     // section header number 节头标项数量
...
int sys_execve(char *filename, char *argv[], char *envp[])
{
    ...
    read(fd, (char *)shstrtab, sptr->sh_size);
