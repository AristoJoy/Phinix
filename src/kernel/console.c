...
static _inline void set_screen(console_t *con)
{
    ...
    out_byte(CRT_DATA_REG, ((con->screen - con->mem_base) >> 9) & 0xff);

    out_byte(CRT_ADDR_REG, CRT_START_ADDR_L); // 开始位置低地址
    out_byte(CRT_DATA_REG, ((con->screen - con->mem_base) >> 1) & 0xff);
}
...
static _inline void set_cursor(console_t *con)
{
    ...
    out_byte(CRT_DATA_REG, ((con->pos - con->mem_base) >> 9) & 0xff);

    out_byte(CRT_ADDR_REG, CRT_CURSOR_L); // 光标低地址
    out_byte(CRT_DATA_REG, ((con->pos - con->mem_base) >> 1) & 0xff);
}
...
static void scroll_up(console_t *con)
{
    if ((con->screen + con->scr_size + con->row_size) >= con->mem_end)
    {
        ...
        con->pos -= (con->screen - con->mem_base); // 光标移动到对应位置
        con->screen = con->mem_base;
    }

    u16 *ptr = (u16 *)(con->screen + con->scr_size); // 没有冒出来的那行清空
...
static _inline void bs(console_t *con)
{
    ...
    con->x--;
    con->pos -= 2;
    *(u16 *)con->pos = con->erase; // 设置前一个字符为空格
}
...
void console_init()
{
    ...
    device_install(DEV_CHAR, DEV_CONSOLE, con, "console", 0, NULL, NULL, console_write);
}
